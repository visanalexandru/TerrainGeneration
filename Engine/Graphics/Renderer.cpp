...
{
    if(to_draw.Get_mesh()&&main_camera.can_see(to_draw) )
    {
        Mesh&mesh=*to_draw.Get_mesh();
        vertices_rendered+=mesh.Get_vertices_size();
        mesh.bind_mesh(false);
        mesh.Get_shader()->set_mat4(camera_view_matrix,view);
        mesh.Get_shader()->set_mat4(main_camera.getProjectionMatrix(),proj);
        mesh.Get_shader()->set_mat4(to_draw.Get_position_matrix(),model);
